...
| W3: public `MojoRuntimeWorker` product | Accept a W2-trusted worker projection and at most one typed immutable input resource, create and reverify private attempt staging, map a socketpair endpoint to child fd 3, spawn one process with either an empty environment or fixed private resource-path and verified staged runtime-library-directory keys, perform bounded protocol I/O, expose generic create/invoke/shutdown, and terminate/reap/remove staging on timeout, cancellation, or crash | Artifact/resource selection policy, vendor runtime library names, model operation meaning, budgets, telemetry interpretation, checkpoint commit, target qualification |
...
| Consuming package | Select an allowed verified artifact and input-resource identity, map domain operations to verified bindings, and own the binding/resource tuple, model-specific runtime library names, attempt policy, budgets, telemetry interpretation, checkpoint commit, and evidence admission | Private staging, private resource/runtime-directory paths, environment keys, POSIX, file descriptors, raw frames/codecs, process signaling/reaping, raw Mojo symbols |
