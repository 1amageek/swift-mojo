...
この文書は、`swift-mojo` が担う言語bridge、生成tooling、artifact検証、
およびverified executable workerのgeneric attempt client要件を定義します。
UI、画面描画、model semantics、vendor framework lifecycleはscope外です。
...
| F-023 | artifact selectorとgeneric worker clientがfresh verificationを共有できる | Implemented and compile-boundary verified; actual-host receipt pending | public verifierはclosed worker semantic/generated/protocol/target/runtime identityをimmutable valueとして返し、絶対bundle pathとconstruction authorityをpackage内へ閉じる。検証APIはbundleを変更または実行せず、W3だけがtrusted projectionをprivate stagingへ消費する |
| F-024 | persistent workerの両generated sideを一つのinput graphから作る | W1/W2 implemented and fixture-verified; host execution pending | exact `MojoInputGraph`と内部`MojoRuntimeProtocolCore`からMojo ABI objectとC worker dispatch/mainを生成し、protocol coreがW3 `MojoRuntimeWorker`向けSwift codec/typesを同じ正本から提供する。source map、pipeline、generated source/object digest、protocol schema、全bindingを`RuntimeWorkerBundle.json` schema 1へ固定し、一方だけのmutationはprepare/verifyを失敗させる |
| F-025 | runtime-dependent workerをdirect-linked executableとして構築する | W2 implemented and fixture-verified; actual host execution pending | 両objectをADR-0010 receipt closureとともにADR-0011 executableへ直接linkし、worker treeにcallable primary libraryを置かず、final executableとsourceに`dlopen`/`dlsym`/`dlclose`経路がないことを検証する |
| F-026 | bounded worker protocol v1をfd 3へ固定する | W1 codec/renderer and W3 transport implemented; actual-host receipt pending | 32-byte little-endian header `UInt32 magic/UInt16 version/UInt16 kind/UInt64 requestID/UInt64 payloadByteCount/UInt64 zero reserved`、manifest-bound payload ceiling、one in-flight request、partial I/O、strict pairing、closed `ready/createSession/sessionCreated/invokeFloat32/invocationResult/shutdownSession/sessionShutdown/shutdownWorker/workerShutdown/failure` kindsを実装し、oversize/truncation/unknown/reorder/trailing dataをdispatch前に拒否する |
| F-027 | W1/W2を生成とread-only verificationへ限定する | Implemented and compile-boundary verified | `MojoArtifactCore`はworker生成・package・verification、`MojoRuntime`はpublic immutable verificationだけを公開し、launcher、transport、mutation、loader、raw symbol/handle、application semanticsを持たない。worker verification valueはpublic construction/decodingを許さない |
| F-028 | W3 lifecycleとcross-target evidenceを分離する | Lifecycle implemented and locally verified; cross-target receipts pending | `MojoRuntimeWorker`のgraceful/cooperative shutdownはlive session/device handleをexactly onceでdestroyする。in-flight hard deadline/crashはdestructor実行を主張せずprocess group termination/reaping、OS process reclamation、app survival、partial output rejection、clean next attemptを証明する。同一semantic identityからApple/NVIDIA別target closureを作りactual hostで別々に実行し、MAX backend名はtarget evidenceのみに使う |
| F-029 | startup identityをpost-link digestから非循環に分離する | W1/W2 identity and W3 ready admission implemented and mutation-tested | protocol/ABI/input graph/bindings/target/compiler/source map/digestを含まないMojo generated source・object/receipt closureからC rendering前に`executionContractDigest`を作りworkerへembedする。C worker source/object、target-closure、executable、manifest digestは必ず除外する。`RuntimeWorkerBundle.json`はexecution contractをそれらのpost-render/link identityへbindし、`ready`はexecution contractとgraph/binding/targetだけを返す |
| F-030 | W3がgeneric public worker clientとprivate process boundaryを所有する | Implemented and compile-boundary verified; actual-host receipt pending | public `MojoRuntimeWorker` productはW2が生成したtrusted worker projectionだけを受理し、private copy後のfresh verification、fd-3 socketpair/spawn、bounded protocol I/O、startup preflight、generic create/invoke/shutdown、deadline/cancel時のterminate/reapを所有する。public APIへpath、filesystem、POSIX、PID、descriptor、raw frame、arbitrary executable launch、model/application semanticsを公開しない |
...
| Worker client | public `MojoRuntimeWorker`がverified projectionからprivate attemptを所有し、consumerにはtyped generic session/invocationだけを公開する |
...
artifact generation and read-only verification only. W3 `MojoRuntimeWorker`
owns private staging with fresh verification, fd-3 process launch, bounded typed
transport, generic session/invocation/shutdown, and terminate/reap recovery.
Model operations, artifact allowlisting, attempt policy, budgets, telemetry,
checkpoint admission, target-device performance, UI integration, and hardware
qualification remain consumer scope. Generic accelerator buffer and
synchronization semantics remain future bridge scope.
