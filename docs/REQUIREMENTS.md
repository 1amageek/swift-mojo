...
| F-023 | artifact selectorとgeneric worker clientがfresh verificationを共有できる | Worker-specific projection implemented; W3 consumption pending | public verifierはclosed worker semantic/generated/protocol/target/runtime identityをimmutable valueとして返し、絶対bundle pathとconstruction authorityをpackage内へ閉じる。検証APIはbundleを変更または実行せず、W3だけがtrusted projectionをprivate stagingへ消費する |
...
| F-026 | bounded worker protocol v1をfd 3へ固定する | W1 codec/renderer implemented; W3 transport pending | 32-byte little-endian header `UInt32 magic/UInt16 version/UInt16 kind/UInt64 requestID/UInt64 payloadByteCount/UInt64 zero reserved`、manifest-bound payload ceiling、one in-flight request、partial I/O、strict pairing、closed `ready/createSession/sessionCreated/invokeFloat32/invocationResult/shutdownSession/sessionShutdown/shutdownWorker/workerShutdown/failure` kindsを実装し、oversize/truncation/unknown/reorder/trailing dataをdispatch前に拒否する |
