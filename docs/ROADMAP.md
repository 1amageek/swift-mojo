...
| 6 — Accelerator bridge | direct-linked persistent worker、generic isolated worker client、device/buffer/event/transfer contracts | Runtime receipt、generic executable bundle、and callable ABI library bundle implemented; ADR-0015 protocol/render、worker bundle、public `MojoRuntimeWorker` client、and generic device conformance are Planned |
...
- worker-specific read-only `MojoRuntime` projection and fresh verifier（planned; no launch or mutation authority）。
