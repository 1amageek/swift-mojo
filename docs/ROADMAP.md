...
| 6 — Accelerator bridge | direct-linked persistent worker、generic isolated worker client、device/buffer/event/transfer contracts | Runtime receipt、generic executable/callable bundles、and ADR-0015 W1〜W3 protocol/render、worker bundle/projection、and isolated client lifecycle implemented; actual-host execution and generic device conformance remain pending |
...
- internal `MojoRuntimeProtocolCore` target with protocol-v1 constants、Float32 payload schemas、and deterministic C/Swift codec rendering（implemented; no public product）。
- same-`MojoInputGraph` generated Mojo object + C worker object directly linked into `RuntimeWorkerBundle.json` schema-1 executable bundle（implemented and fixture-verified; actual host execution pending; no `dlopen`/`dlsym`/`dlclose`）。
- worker-specific read-only `MojoRuntime` projection and fresh verifier（implemented; no launch or mutation authority）。
- public generic `MojoRuntimeWorker` product accepting only that trusted projection and privately owning staging/reverification、fd-3 socketpair/spawn、bounded protocol I/O、one-session invocation、graceful exactly-once teardown、timeout/cancel terminate+reap、hard-crash OS reclamation + clean-next-attempt recovery（implemented with local lifecycle fixtures; actual-host production receipt pending; no public filesystem/POSIX/raw-frame or application semantics）。
