...
| 6 — Accelerator bridge | direct-linked persistent worker、generic isolated worker client、device/buffer/event/transfer contracts | Runtime receipt、generic executable/callable bundles、ADR-0015 W1 protocol/render、W2 worker bundle/read-only projection implemented; W3 client、actual host execution、and generic device conformance are planned |
...
- public generic `MojoRuntimeWorker` product accepting only that trusted projection and privately owning staging/reverification、fd-3 socketpair/spawn、bounded protocol I/O、one-session invocation、graceful exactly-once teardown、timeout/cancel terminate+reap、hard-crash OS reclamation + clean-next-attempt recovery（planned; no public filesystem/POSIX/raw-frame or application semantics）。
