...
@Suite("Mutable Mojo Float64 buffer artifact")
struct MojoFloat64BufferArtifactTests {
    @Test(.timeLimit(.minutes(1)))
    func preparesScopedFloat64BufferABIAndTypedRegistry() throws {
        ...
        defer {
            do {
                ...
                Issue.record("Failed to remove Float64 buffer fixture: \(error)")
            }
        }
        ...
        let identity = try MojoArtifactIdentity(targetName: "Dynamics")
        ...
        #expect(generatedMojo.contains("Pointer[Float64, ImmUntrackedOrigin]"))
        #expect(generatedMojo.contains("Pointer[Float64, MutUntrackedOrigin]"))
        #expect(generatedMojo.contains("_call_f64_buffer_f64_buffer_i32"))
        #expect(
            header.contains(
                "int32_t \(identity.symbolPrefix)_call_f64_buffer_f64_buffer_i32("
            )
        )
