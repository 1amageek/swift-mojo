...
package struct MojoArtifactPreparer: Sendable {
    ...
    if let archives = groups[.appleXCFramework] {
      ...
      artifacts.append(
        MojoArtifactManifest.Artifact(
          ...
          digest: try MojoCanonicalDigest.tree(
            at: artifactURL,
  ...
  private func packageAppleLibraries(
    ...
    archiveURL: URL,
    style: MojoStaticFrameworkLayout.Style
  )] {
    ...
    for (index, key) in groups.keys.sorted().enumerated() {
      ...
      guard let firstTarget = group.first?.0 else {
      ...
      guard group.count > 1 else {
        ...
        result.append((archiveURL: archiveURL, style: style))
        continue
      }
      ...
      result.append((archiveURL: combinedArchiveURL, style: style))
    }
    return result
  }
  ...
  private func cachedManifest(
    ...
    guard fileManager.fileExists(atPath: options.manifestURL.path),
      ...
      MojoRegularFile.isValid(at: options.sourceMapURL)
    else {
      return nil
    }
    ...
    guard artifacts.count == expectedAdapters.count,
      ...
      manifest.artifactDigest
        == MojoArtifactManifest.digest(artifacts: artifacts)
    else {
      return nil
    }
    for artifact in artifacts {
      ...
      do {
        ...
        case .appleXCFramework:
          ...
          for slice in artifactSlices {
            ...
            try MojoStaticFrameworkLayout.validateFramework(
              at: MojoStaticFrameworkLayout.frameworkURL(
                in: sliceURL,
                identity: options.identity
              ),
              identity: options.identity,
        ...
        let allowedSymbolicLinks =
          artifact.adapter == .appleXCFramework
          ? try MojoStaticFrameworkLayout.allowedSymbolicLinks(
            identity: options.identity,
            slices: artifactSlices
          )
          : [:]
        guard
          try MojoCanonicalDigest.tree(
            at: artifactURL,
            allowedSymbolicLinks: allowedSymbolicLinks
          )
            == artifact.digest
        else {
          return nil
